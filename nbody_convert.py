...
def main():
	...
	for i in xrange(nbodies):
		x = float(contents[2 + i  * 6]) / meters_per_au
		y = float(contents[3 + i  * 6]) / meters_per_au
		vx = float(contents[4 + i  * 6]) * seconds_per_day / meters_per_au 
		vy = float(contents[5 + i  * 6]) * seconds_per_day / meters_per_au 
		m = float(contents[6 + i  * 6]) / kg_per_earthmass
